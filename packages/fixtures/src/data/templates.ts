...
export function buildTemplates(): PrintTemplate[] {
  ...
  const stripPhotos = (locale: 'es' | 'en'): TemplateElement[] => [
    ...
    { id: 'date', type: 'token', token: 'date', box: box(3, 146, 44.8, 4), style: { fontFamily: 'sans-serif', sizePt: 7, color: '#111111', align: 'center', weight: 'normal' }, format: locale === 'es' ? 'dd/MM/yyyy' : 'MM/dd/yyyy', zIndex: 2 },
  ];
  return [
    ...
    PrintTemplate.parse({
      id: T.strip2x6, organizationId: ID.org.unaDeTodos, name: L('Tira 2x6 cuatro fotos', '2x6 strip, four photos'), kind: 'strip_vertical', paperSize: '2x6in-strip', canvas: strip, orientation: 'portrait', photoSlots: 4,
      elements: stripPhotos('es'),
      variants: [{ key: 'en', selector: { locale: 'en' }, elements: stripPhotos('en') }],
      tags: ['strip', 'fun'], ...audit(ID.user.adminUnaDeTodos),
    }),
    PrintTemplate.parse({
      id: T.grid4x6, organizationId: ID.org.unaDeTodos, name: L('Cuadrícula 4x6 cuatro fotos', '4x6 grid, four photos'), kind: 'grid', paperSize: '4x6in', canvas: p4x6, orientation: 'portrait', photoSlots: 4,
      elements: [
      ...
      tags: ['grid', 'birthday'], ...audit(ID.user.adminUnaDeTodos),
    }),
    PrintTemplate.parse({
      id: T.single4x6, organizationId: ID.org.unaDeTodos, name: L('Foto sencilla 4x6', 'Single 4x6 photo'), kind: 'single', paperSize: '4x6in', canvas: p4x6, orientation: 'portrait', photoSlots: 1,
      elements: [
        { id: 'bg', type: 'background', box: box(0, 0, p4x6.widthMm, p4x6.heightMm), color: '#FFFFFF', zIndex: 0 },
        ...
        { id: 'location', type: 'token', token: 'locationName', box: box(4, 143, 93.6, 5), style: { fontFamily: 'sans-serif', sizePt: 8, color: '#111111', align: 'center', weight: 'normal' }, zIndex: 2 },
      ] satisfies TemplateElement[],
      tags: ['single', 'portrait'], ...audit(ID.user.adminUnaDeTodos),
    }),
    PrintTemplate.parse({
      id: T.thermal58, organizationId: ID.org.unaDeTodos, name: L('Ticket térmico 58 mm', '58 mm thermal ticket'), kind: 'thermal_receipt', paperSize: '58mm-thermal', canvas: thermal, orientation: 'portrait', photoSlots: 1,
      elements: [
      ...
      tags: ['thermal', 'receipt'], ...audit(ID.user.adminUnaDeTodos),
    }),
    PrintTemplate.parse({
      id: T.postcard5x7, organizationId: ID.org.unaDeTodos, name: L('Postal navideña 5x7', 'Christmas postcard 5x7'), kind: 'postcard', paperSize: '5x7in', canvas: p5x7, orientation: 'portrait', photoSlots: 1,
      elements: [
      ...
      tags: ['postcard', 'navidad'], ...audit(ID.user.adminUnaDeTodos),
    }),
  ];
}
