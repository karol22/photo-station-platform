...
export function buildExperiences(): Experience[] {
  return [
    Experience.parse({
      id: ID.experience.bestFriends, organizationId: ID.org.lumina, key: 'best_friends', name: L('Mejores amigos', 'Best friends'), theme: 'best_friends',
      ...
      stickerAssetIds: [A.stickerStar, A.stickerHeart], editingPresetIds: [ID.editingPreset.vivid, ID.editingPreset.retro, ID.editingPreset.bwSoft], templateId: ID.template.strip2x6, status: 'active', ...audit(ID.user.adminLumina),
    }),
    Experience.parse({
      ...
      stickerAssetIds: [A.stickerHeart], editingPresetIds: [ID.editingPreset.vivid, ID.editingPreset.bwSoft], templateId: ID.template.strip2x6, status: 'active', ...audit(ID.user.owner),
    }),
    Experience.parse({
      id: ID.experience.birthday, organizationId: ID.org.lumina, key: 'birthday', name: L('Cumpleaños', 'Birthday'), theme: 'birthday',
      ...
      selection: { min: 3, max: 3, allowReorder: true, allowCompare: true },
      frameAssetIds: [A.frameBirthday], stickerAssetIds: [A.stickerStar], overlayAssetIds: [A.bgConfetti], editingPresetIds: [ID.editingPreset.vivid], templateId: ID.template.grid4x6, status: 'active', ...audit(ID.user.adminLumina),
    }),
    Experience.parse({
      id: ID.experience.christmas, organizationId: ID.org.lumina, key: 'christmas', name: L('Navidad', 'Christmas'), theme: 'christmas',
      ...
      season: { start: '2026-12-01T06:00:00Z', end: '2027-01-06T06:00:00Z' }, status: 'active', ...audit(ID.user.adminLumina),
    }),
  ];
}
...
export function buildEditingPresets(): EditingPreset[] {
  return [
    EditingPreset.parse({ id: ID.editingPreset.vivid, organizationId: ID.org.lumina, key: 'vivid', name: L('Vívido', 'Vivid'), ops: [{ op: 'saturation', params: { amount: 0.3 } }, { op: 'contrast', params: { amount: 0.15 } }], documentSafe: false, tags: ['color'], ...audit(ID.user.adminLumina) }),
    EditingPreset.parse({ id: ID.editingPreset.retro, organizationId: ID.org.lumina, key: 'retro', name: L('Retro', 'Retro'), ops: [{ op: 'temperature', params: { amount: 0.3 } }, { op: 'saturation', params: { amount: -0.2 } }, { op: 'vignette', params: { strength: 0.4 } }], documentSafe: false, tags: ['film'], ...audit(ID.user.adminLumina) }),
    EditingPreset.parse({ id: ID.editingPreset.bwSoft, organizationId: ID.org.lumina, key: 'bw_soft', name: L('Blanco y negro suave', 'Soft black & white'), ops: [{ op: 'grayscale', params: {} }, { op: 'contrast', params: { amount: -0.1 } }, { op: 'brightness', params: { amount: 0.05 } }], documentSafe: false, tags: ['bw'], ...audit(ID.user.adminLumina) }),
