 ...
 * La organización real (Una de Todos) borra al terminar: `privacy.defaultRetentionPolicyId` es
 * `ret_delete_on_finish` y ninguna imagen sobrevive a la sesión. `customer.handoff` no lleva override:
 * la membresía es una etapa posterior, así que la función se queda en su modo oculto por defecto.
 */
...
export function buildRetentionPolicies(): RetentionPolicy[] {
  return [
    RetentionPolicy.parse({ id: ID.retention.deleteOnFinish, name: L('Borrar al terminar', 'Delete on finish'), mode: 'none', deleteIncomplete: true, appliesToKinds: [], customerText: L('Tu foto se borra de la máquina en cuanto termina la sesión: no queda copia aquí ni en ningún otro lado.', 'Your photo is deleted from the machine as soon as the session ends: no copy stays here or anywhere else.'), leavesDevice: false }),
...
export function buildConfigLayers(): ConfigLayer[] {
  ...
  return [
    ...
    ConfigLayer.parse({
      id: C.orgUnaDeTodos, level: 'organization', entityId: ID.org.unaDeTodos,
      values: {
        'branding.publicName': 'Una de Todos', 'branding.logoAssetId': ID.asset.logoUnaDeTodos, 'branding.printLogoAssetId': ID.asset.logoUnaDeTodos,
        'branding.palette.primary': '#FF6FA5', 'branding.palette.secondary': '#111111', 'branding.palette.accent': '#FFC24A', 'branding.palette.background': '#F3EEE4', 'branding.palette.text': '#111111',
        'branding.attractImageAssetIds': [ID.asset.attractFamily], 'branding.footerText': 'Una de Todos · Fotos que nos juntan',
        'branding.completionMessage': '¡Qué buena una de todos! Llévatela y vuelve cuando quieras.',
        'legal.privacyNotice': 'Una de Todos usa tu foto sólo para imprimirla y la borra de la máquina al terminar la sesión.', 'legal.terms': 'Verifica la foto antes de imprimir; no hay reembolsos por errores del cliente.', 'legal.supportContact': 'soporte@unadetodos.demo',
        'privacy.defaultRetentionPolicyId': ID.retention.deleteOnFinish, 'timing.idleTimeoutSec': 60, 'kiosk.locales': ['es', 'en'], 'techPanel.pinHash': sha256Hex(TECH_PANEL_PIN),
      },
      locks: [
        { key: 'branding.palette.primary', policy: 'mandatory', setBy: 'organization', setById: ID.org.unaDeTodos },
        { key: 'branding.palette.secondary', policy: 'mandatory', setBy: 'organization', setById: ID.org.unaDeTodos },
        { key: 'branding.palette.accent', policy: 'mandatory', setBy: 'organization', setById: ID.org.unaDeTodos },
        { key: 'timing.idleTimeoutSec', policy: 'range', range: { min: 30, max: 180 }, setBy: 'organization', setById: ID.org.unaDeTodos },
      ],
      version: 4, updatedAt: UPDATED_AT, updatedBy: ID.user.adminUnaDeTodos,
    }),
    ConfigLayer.parse({
    ...
    ConfigLayer.parse({
      ...
      values: { 'branding.footerText': 'Una de Todos Norte · Monterrey', 'kiosk.locales': ['es'], 'branding.palette.primary': '#FF0000', 'branding.secondaryLogoAssetId': ID.asset.logoNorte },
    ...
    ConfigLayer.parse({ id: C.mchHotel, level: 'machine', entityId: ID.machine.hotel, values: { 'payment.businessMode': 'courtesy', 'kiosk.showPricesOnIdle': false }, locks: [], version: 1, updatedAt: UPDATED_AT, updatedBy: ID.user.adminUnaDeTodos }),
    ...
    ConfigLayer.parse({ id: C.mchPremium, level: 'machine', entityId: ID.machine.premium, values: { 'payment.terminalAdapter': 'mock', 'payment.businessMode': 'paid', 'kiosk.volume': 60 }, locks: [], version: 1, updatedAt: UPDATED_AT, updatedBy: ID.user.adminUnaDeTodos }),
  ];
}
...
export function buildEntitlements(): Entitlement[] {
  return [
    Entitlement.parse({ id: ID.entitlement.unaDeTodos, scope: { level: 'organization', id: ID.org.unaDeTodos }, planId: ID.plan.enterprise, startsAt: '2026-01-15T12:00:00Z', notes: 'Plan enterprise anual.' }),
...
export function buildFeatureOverrides(): FeatureOverride[] {
  return [
    FeatureOverride.parse({ id: ID.featureOverride.aiUnaDeTodosHidden, key: 'ai.experiences', scope: { level: 'organization', id: ID.org.unaDeTodos }, mode: 'hidden', reason: 'Proveedor de IA pendiente de contrato; oculto salvo en el piloto Norte.', setBy: ID.user.adminUnaDeTodos, setAt: '2026-08-01T12:00:00Z' }),
    FeatureOverride.parse({ id: ID.featureOverride.aiNorte, key: 'ai.experiences', scope: { level: 'franchise', id: ID.franchise.norte }, mode: 'enabled', reason: 'Piloto de IA en franquicia Norte.', setBy: ID.user.adminUnaDeTodos, setAt: '2026-08-15T12:00:00Z' }),
    FeatureOverride.parse({ id: ID.featureOverride.deliveryUnaDeTodos, key: 'delivery.digital', scope: { level: 'organization', id: ID.org.unaDeTodos }, mode: 'coming_soon', reason: 'Entrega digital anunciada para 2027.', setBy: ID.user.adminUnaDeTodos, setAt: '2026-08-01T12:00:00Z' }),
