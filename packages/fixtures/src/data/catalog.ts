...
function documentProduct(over: { id: string; organizationId?: string; internalName: string; name: [string, string]; description: [string, string]; presetId: string; templateId: string; paper: '4x6in' | '5x7in'; price: Money; category?: 'documents' | 'graduation'; priority: number; tags: string[] }): Product {
  ...
  return Product.parse({
    id: over.id,
    organizationId: over.organizationId ?? ID.org.lumina,
    ...
    privacyNote: L('Tus fotos se eliminan de la máquina al terminar según la política de retención.', 'Your photos are deleted from the machine when you finish, per the retention policy.'),
...
export function buildProducts(): Product[] {
  return [
    documentProduct({ id: P.docUniversity, internalName: 'doc-universitaria', name: ['Foto universitaria', 'University ID photo'], description: ['Fotografía infantil/adulto para credencial y trámites universitarios en México.', 'Photo for university credentials and paperwork in Mexico.'], presetId: ID.preset.mxUniversity, templateId: T.sheet4x6, paper: '4x6in', price: mxn(8000), priority: 10, tags: ['documentos', 'universidad'] }),
    documentProduct({ id: P.docGraduation, internalName: 'doc-graduacion', name: ['Foto de graduación', 'Graduation photo'], description: ['Retrato formal tamaño diploma para ceremonias de graduación.', 'Formal diploma-size portrait for graduation ceremonies.'], presetId: ID.preset.mxGraduation, templateId: T.sheet5x7, paper: '5x7in', price: mxn(9000), category: 'graduation', priority: 20, tags: ['documentos', 'graduacion'] }),
    documentProduct({ id: P.docChild, internalName: 'doc-infantil', name: ['Foto infantil', 'Child ID photo'], description: ['Fotografía tamaño infantil con fondo neutro para trámites escolares.', 'Child-size photo with neutral background for school paperwork.'], presetId: ID.preset.mxChild, templateId: T.sheet4x6, paper: '4x6in', price: mxn(7000), priority: 30, tags: ['documentos', 'infantil'] }),
    documentProduct({ id: P.docVisaUsa, internalName: 'doc-visa-usa', name: ['Foto visa EE. UU. (2x2 in)', 'US visa photo (2x2 in)'], description: ['Fotografía 2x2 pulgadas con fondo blanco, sin lentes y sin sonreír.', '2x2 inch photo on white background, no glasses, no smile.'], presetId: ID.preset.usVisa, templateId: T.sheet4x6Visa, paper: '4x6in', price: mxn(12000), priority: 40, tags: ['documentos', 'visa'] }),
    Product.parse({
      id: P.portraitPro, organizationId: ID.org.lumina, internalName: 'retrato-pro', displayName: L('Retrato profesional', 'Professional portrait'), category: 'professional_portrait', kind: 'portrait',
      ...
      hardwareRequirements: ['camera.primary', 'printer.photo', 'lighting.controllable'], requiredFeatures: ['printing.photo', 'editing.local'], retentionPolicyId: ID.retention.temp30min, status: 'active', priority: 50, tags: ['retrato'], ...audit(ID.user.adminLumina),
    }),
    Product.parse({
      id: P.friendsStrip, organizationId: ID.org.lumina, internalName: 'tira-amigos', displayName: L('Tira de amigos', 'Friends strip'), category: 'fun', kind: 'entertainment',
      ...
      coverAssetId: ID.asset.bgConfetti, estimatedDurationSec: 240, captureCount: 4, printCount: 2,
      ...
      hardwareRequirements: ['camera.primary', 'printer.photo'], requiredFeatures: ['entertainment.mode', 'editing.creative', 'printing.photo'], retentionPolicyId: ID.retention.temp30min, status: 'active', priority: 60, tags: ['diversion', 'tira'], ...audit(ID.user.adminLumina),
    }),
    Product.parse({
      id: P.receiptPhoto, organizationId: ID.org.lumina, internalName: 'foto-recibo', displayName: L('Foto recibo', 'Receipt photo'), category: 'receipt_photo', kind: 'entertainment',
      ...
      hardwareRequirements: ['camera.primary', 'printer.thermal'], requiredFeatures: ['printing.thermal', 'entertainment.mode'], retentionPolicyId: ID.retention.deleteOnFinish, status: 'active', priority: 70, tags: ['termica', 'cafeteria'], ...audit(ID.user.adminLumina),
    }),
    Product.parse({
      id: P.christmasPortrait, organizationId: ID.org.lumina, internalName: 'retrato-navidad', displayName: L('Retrato navideño', 'Christmas portrait'), category: 'themed_portrait', kind: 'entertainment',
      ...
      hardwareRequirements: ['camera.primary', 'printer.photo'], requiredFeatures: ['entertainment.mode', 'campaigns', 'printing.photo'], startsAt: '2026-12-01T06:00:00Z', endsAt: '2027-01-06T06:00:00Z', retentionPolicyId: ID.retention.temp30min, status: 'active', priority: 80, tags: ['navidad', 'campaña'], ...audit(ID.user.adminLumina),
    }),
    Product.parse({
      id: P.aiAnime, organizationId: ID.org.lumina, internalName: 'ia-anime', displayName: L('Retrato estilo anime (IA)', 'Anime-style portrait (AI)'), category: 'ai_future', kind: 'ai',
      ...
      hardwareRequirements: ['camera.primary', 'printer.photo', 'connectivity.online'], requiredFeatures: ['ai.experiences', 'printing.photo'], retentionPolicyId: ID.retention.derivatives24h, status: 'active', priority: 90, tags: ['ia'], ...audit(ID.user.adminLumina),
    }),
...
export function buildProductAvailabilities(): ProductAvailability[] {
  return [
    ProductAvailability.parse({ id: ID.availability.aiOrgOff, productId: P.aiAnime, scope: { level: 'organization', id: ID.org.lumina }, enabled: false, updatedAt: UPDATED_AT, updatedBy: ID.user.adminLumina }),
    ProductAvailability.parse({ id: ID.availability.aiNorteOn, productId: P.aiAnime, scope: { level: 'franchise', id: ID.franchise.norte }, enabled: true, priorityOverride: 95, updatedAt: UPDATED_AT, updatedBy: ID.user.franqNorte }),
  ];
}
...
export function buildPriceRules(products: Product[]): PriceRule[] {
  ...
  return [
    ...
    PriceRule.parse({ id: ID.priceRule.portraitProPremium, productId: P.portraitPro, scope: { level: 'machine', id: ID.machine.premium }, price: mxn(18000), priority: 0, updatedAt: UPDATED_AT, updatedBy: ID.user.adminLumina }),
  ];
}
...
export function buildPromotions(): Promotion[] {
  return [
    Promotion.parse({
      id: ID.promotion.strip2x1Tuesday, organizationId: ID.org.lumina, franchiseId: ID.franchise.norte, name: L('2x1 en tira los martes', '2-for-1 strips on Tuesdays'), type: 'second_print', value: 100,
    ...
    Promotion.parse({
      id: ID.promotion.cafe10pct, organizationId: ID.org.lumina, franchiseId: ID.franchise.norte, name: L('10 % en Café Aurora', '10% off at Café Aurora'), type: 'percent_discount', value: 10,
      productIds: [], scope: { level: 'location', id: ID.location.cafe }, priority: 5, status: 'active', ...audit(ID.user.franqNorte),
    }),
    Promotion.parse({
      id: ID.promotion.code10, organizationId: ID.org.lumina, name: L('Código PROMO10', 'PROMO10 code'), type: 'code', value: 10, code: 'PROMO10',
      productIds: [], scope: { level: 'organization', id: ID.org.lumina }, window: { start: '2026-09-01T06:00:00Z', end: '2026-12-31T06:00:00Z' }, priority: 1, status: 'active', ...audit(ID.user.adminLumina),
    }),
  ];
}
