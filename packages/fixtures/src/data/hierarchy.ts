...
export function buildOrganizations(): Organization[] {
  return [
    Organization.parse({
      ...
      slug: 'lumina',
      name: 'Lumina Foto',
      legalName: 'Lumina Foto Digital S.A. de C.V.',
      country: 'MX',
      currency: 'MXN',
      timezone: TZ_MX,
      defaultLocale: 'es',
      locales: ['es', 'en'],
      support: { name: 'Soporte Lumina', phone: '+52 55 0000 0001', email: 'soporte@lumina.demo' },
      legal: {
        privacyNotice: L('Lumina Foto trata tus fotografías sólo para producir tu impresión y las elimina según la política mostrada en pantalla.', 'Lumina Foto processes your photos only to produce your print and deletes them according to the on-screen policy.'),
...
export function buildFranchises(): Franchise[] {
  return [
    Franchise.parse({ id: ID.franchise.norte, organizationId: ID.org.lumina, name: 'Lumina Norte', legalName: 'Fotografía del Norte S.A. de C.V.', contact: { name: 'Franquiciatario Norte', email: 'franq@norte.demo', phone: '+52 81 0000 0003' }, status: 'active', notes: 'Franquicia piloto: universidades y cafeterías de Monterrey.', ...audit(ID.user.adminLumina), createdAt: '2026-02-01T12:00:00Z' }),
    Franchise.parse({ id: ID.franchise.bajio, organizationId: ID.org.lumina, name: 'Lumina Bajío', legalName: 'Imagen Bajío S. de R.L.', contact: { name: 'Franquiciatario Bajío', email: 'franq@bajio.demo', phone: '+52 442 000 0004' }, status: 'active', notes: 'Cines y hoteles en Querétaro y León.', ...audit(ID.user.adminLumina), createdAt: '2026-04-01T12:00:00Z' }),
  ];
}

export function buildTerritories(): Territory[] {
  return [
    Territory.parse({ id: ID.territory.norteNuevoLeon, franchiseId: ID.franchise.norte, country: 'MX', state: 'Nuevo León', city: 'Monterrey', commercialZone: 'Área metropolitana de Monterrey', contractualText: 'Exclusividad en campus universitarios del área metropolitana.', startDate: '2026-02-01T00:00:00Z', exclusive: true, ...audit(ID.user.adminLumina) }),
    Territory.parse({ id: ID.territory.bajioQroGto, franchiseId: ID.franchise.bajio, country: 'MX', state: 'Querétaro y Guanajuato', commercialZone: 'Corredor Querétaro–León', startDate: '2026-04-01T00:00:00Z', exclusive: false, ...audit(ID.user.adminLumina) }),
  ];
}

export function buildRegions(): Region[] {
  return [
    Region.parse({ id: ID.region.norte, organizationId: ID.org.lumina, franchiseId: ID.franchise.norte, name: 'Norte', country: 'MX', timezone: TZ_MTY, ...audit() }),
    Region.parse({ id: ID.region.bajio, organizationId: ID.org.lumina, franchiseId: ID.franchise.bajio, name: 'Bajío', country: 'MX', timezone: TZ_MX, ...audit() }),
    Region.parse({ id: ID.region.centro, organizationId: ID.org.lumina, name: 'Centro', country: 'MX', timezone: TZ_MX, ...audit() }),
...
export function buildLocations(): Location[] {
  return [
    Location.parse({
      id: ID.location.university, organizationId: ID.org.lumina, franchiseId: ID.franchise.norte, regionId: ID.region.norte,
    ...
    Location.parse({
      id: ID.location.cafe, organizationId: ID.org.lumina, franchiseId: ID.franchise.norte, regionId: ID.region.norte,
    ...
    Location.parse({
      id: ID.location.mallCdmx, organizationId: ID.org.lumina, regionId: ID.region.centro,
      ...
      status: 'active', installedAt: '2026-06-20T18:00:00Z', tags: ['mall', 'premium', PILOT_TAG], ...audit(ID.user.adminLumina),
    }),
    Location.parse({
      id: ID.location.cinema, organizationId: ID.org.lumina, franchiseId: ID.franchise.bajio, regionId: ID.region.bajio,
      ...
      status: 'active', installedAt: '2026-07-01T17:00:00Z', tags: ['cine', 'bajio'], ...audit(ID.user.adminLumina),
    }),
    Location.parse({
      id: ID.location.hotel, organizationId: ID.org.lumina, franchiseId: ID.franchise.bajio, regionId: ID.region.bajio,
      ...
      status: 'active', installedAt: '2026-07-15T17:00:00Z', tags: ['hotel', 'cortesia', 'bajio'], ...audit(ID.user.adminLumina),
    }),
    Location.parse({
...
export const PRINTERS = {
  photo4x6: { id: ID.printer.photo, name: 'Impresora fotográfica', type: 'photo', paperSizes: ['4x6in', '5x7in'], color: true, consumableType: 'photo_paper', priority: 0 } satisfies PrinterDefinition,
...
export function buildHardwareProfiles(): HardwareProfile[] {
  return [
    HardwareProfile.parse({
      ...
      printers: [PRINTERS.photo4x6], paperSizes: ['4x6in', '5x7in'],
...
export function buildBlueprints(): Blueprint[] {
  ...
  return [
    Blueprint.parse({
      id: ID.blueprint.docUniversity, organizationId: ID.org.lumina, key: 'doc_university', name: L('Documental universitaria', 'University ID station'),
      description: L('Estación de fotos para trámites en campus.', 'ID photo station for campus paperwork.'),
      hardwareProfileId: ID.hardwareProfile.docStation, productIds: [P.docUniversity, P.docGraduation, P.docChild, P.docVisaUsa, P.portraitPro],
      configValues: { 'kiosk.showPricesOnIdle': true, 'timing.idleTimeoutSec': 60, 'branding.tone': 'formal' },
      featureModes: { 'documents.mode': 'enabled', 'entertainment.mode': 'hidden' },
      maintenanceChecklistId: ID.checklist.docStation, ...audit(ID.user.adminLumina),
    }),
    Blueprint.parse({
      id: ID.blueprint.thermalCafe, organizationId: ID.org.lumina, key: 'thermal_cafe', name: L('Kiosco térmico de cafetería', 'Café thermal kiosk'),
      description: L('Foto recibo en blanco y negro para cafeterías.', 'Black-and-white receipt photo for cafés.'),
      ...
      featureModes: { 'documents.mode': 'hidden', 'printing.thermal': 'enabled' },
      maintenanceChecklistId: ID.checklist.thermalKiosk, ...audit(ID.user.adminLumina),
    }),
    Blueprint.parse({
      id: ID.blueprint.premiumBooth, organizationId: ID.org.lumina, key: 'premium_booth', name: L('Cabina premium', 'Premium booth'),
      description: L('Cabina de entretenimiento con pago en terminal.', 'Entertainment booth with terminal payments.'),
      ...
      configValues: { 'timing.idleTimeoutSec': 90, 'kiosk.volume': 60, 'branding.tone': 'friendly' },
      featureModes: { 'entertainment.mode': 'enabled', 'payments.terminal': 'enabled' },
      ...audit(ID.user.adminLumina),
    }),
    Blueprint.parse({
      id: ID.blueprint.demoUnit, key: 'demo_unit', name: L('Unidad demo', 'Demo unit'),
      description: L('Recorrido completo sin venta para ferias y capacitación.', 'Full journey without a sale for fairs and training.'),
      hardwareProfileId: ID.hardwareProfile.docStation, productIds: [P.docUniversity, P.portraitPro, P.friendsStrip],
      configValues: { 'payment.businessMode': 'demo', 'kiosk.showPricesOnIdle': false, 'timing.idleTimeoutSec': 120 },
      featureModes: { 'kiosk.demoMode': 'enabled' },
      ...audit(ID.user.owner),
    }),
  ];
}
...
const seeds: MachineSeed[] = [
  { id: ID.machine.doc, code: 'LUM-NTE-001', name: 'Documental Universidad del Norte', locationId: ID.location.university, franchiseId: ID.franchise.norte, regionId: ID.region.norte, hardwareProfileId: ID.hardwareProfile.docStation, blueprintId: ID.blueprint.docUniversity, status: 'active', capabilities: caps(CAPS.docStation), printers: [PRINTERS.photo4x6], softwareVersion: SOFTWARE_VERSIONS.v020, online: true, lastSeenAt: minutesAgo(1), installedAt: '2026-02-15T16:00:00Z', timezone: TZ_MTY, tags: ['universidad'], notes: 'Máquina que simula el agente por defecto (PSP_STATION_MACHINE_ID).' },
  { id: ID.machine.thermal, code: 'LUM-NTE-002', name: 'Kiosco térmico Café Aurora', locationId: ID.location.cafe, franchiseId: ID.franchise.norte, regionId: ID.region.norte, hardwareProfileId: ID.hardwareProfile.thermalKiosk, blueprintId: ID.blueprint.thermalCafe, status: 'active', capabilities: caps(CAPS.thermalKiosk), printers: [PRINTERS.thermal58], softwareVersion: SOFTWARE_VERSIONS.v010, online: true, lastSeenAt: minutesAgo(2), installedAt: '2026-05-10T17:00:00Z', timezone: TZ_MTY, tags: ['cafeteria'] },
  { id: ID.machine.premium, code: 'LUM-CEN-001', name: 'Cabina premium Plaza Meridiano', locationId: ID.location.mallCdmx, regionId: ID.region.centro, hardwareProfileId: ID.hardwareProfile.premiumBooth, blueprintId: ID.blueprint.premiumBooth, status: 'active_with_warnings', capabilities: caps(CAPS.premiumBooth, { 'printer.photo': 'Sin papel: incidencia abierta' }), printers: [PRINTERS.photoPremium], softwareVersion: SOFTWARE_VERSIONS.v010, targetSoftwareVersion: SOFTWARE_VERSIONS.v020, online: true, lastSeenAt: minutesAgo(1), installedAt: '2026-06-20T19:00:00Z', timezone: TZ_MX, tags: [PILOT_TAG, 'premium'], notes: 'Piloto de la release 0.2.0; papel fotográfico agotado.' },
  { id: ID.machine.cinema, code: 'LUM-BAJ-001', name: 'Documental Cinema Juriquilla', locationId: ID.location.cinema, franchiseId: ID.franchise.bajio, regionId: ID.region.bajio, hardwareProfileId: ID.hardwareProfile.docStation, blueprintId: ID.blueprint.docUniversity, status: 'maintenance', capabilities: caps(CAPS.docStation, { 'camera.primary': 'Cámara no responde; en revisión' }), printers: [PRINTERS.photo4x6], softwareVersion: SOFTWARE_VERSIONS.v010, online: true, lastSeenAt: minutesAgo(5), installedAt: '2026-07-01T18:00:00Z', timezone: TZ_MX, tags: ['cine'] },
  { id: ID.machine.hotel, code: 'LUM-BAJ-002', name: 'Cabina Hotel Casa Real', locationId: ID.location.hotel, franchiseId: ID.franchise.bajio, regionId: ID.region.bajio, hardwareProfileId: ID.hardwareProfile.premiumBooth, blueprintId: ID.blueprint.premiumBooth, status: 'active', capabilities: caps(CAPS.premiumBooth), printers: [PRINTERS.photoPremium], softwareVersion: SOFTWARE_VERSIONS.v010, online: true, lastSeenAt: minutesAgo(3), installedAt: '2026-07-15T18:00:00Z', timezone: TZ_MX, tags: ['hotel', 'cortesia'] },
  { id: ID.machine.demo, code: 'LUM-DEMO-001', name: 'Unidad demo itinerante', hardwareProfileId: ID.hardwareProfile.docStation, blueprintId: ID.blueprint.demoUnit, status: 'demo', capabilities: caps(CAPS.docStation), printers: [PRINTERS.photo4x6], softwareVersion: SOFTWARE_VERSIONS.v030pilot1, releaseChannel: 'pilot', online: true, lastSeenAt: minutesAgo(10), installedAt: '2026-08-01T12:00:00Z', timezone: TZ_MX, tags: ['demo'], notes: 'Sin ubicación fija; canal pilot.' },
  { id: ID.machine.bajio1, code: 'LUM-BAJ-003', name: 'Cabina Cinema Juriquilla', locationId: ID.location.cinema, franchiseId: ID.franchise.bajio, regionId: ID.region.bajio, hardwareProfileId: ID.hardwareProfile.premiumBooth, blueprintId: ID.blueprint.premiumBooth, status: 'active', capabilities: caps(CAPS.premiumBooth), printers: [PRINTERS.photoPremium], softwareVersion: SOFTWARE_VERSIONS.v020, online: true, lastSeenAt: minutesAgo(1), installedAt: '2026-07-01T18:30:00Z', timezone: TZ_MX, tags: [PILOT_TAG, 'cine'] },
  { id: ID.machine.bajio2, code: 'LUM-BAJ-004', name: 'Documental Hotel Casa Real', locationId: ID.location.hotel, franchiseId: ID.franchise.bajio, regionId: ID.region.bajio, hardwareProfileId: ID.hardwareProfile.docStation, blueprintId: ID.blueprint.docUniversity, status: 'disconnected', capabilities: caps(CAPS.docStation), printers: [PRINTERS.photo4x6], softwareVersion: SOFTWARE_VERSIONS.v010, online: false, lastSeenAt: daysAgo(2, 3), installedAt: '2026-07-15T18:30:00Z', timezone: TZ_MX, tags: ['hotel'], notes: 'Sin heartbeat desde hace dos días.' },
  { id: ID.machine.terminal, code: 'LUM-CEN-002', name: 'Kiosco térmico Plaza Meridiano', locationId: ID.location.mallCdmx, regionId: ID.region.centro, hardwareProfileId: ID.hardwareProfile.thermalKiosk, blueprintId: ID.blueprint.thermalCafe, status: 'out_of_service', capabilities: caps(CAPS.thermalKiosk, { 'printer.thermal': 'Cabezal térmico dañado; esperando refacción' }), printers: [PRINTERS.thermal58], softwareVersion: SOFTWARE_VERSIONS.v010, online: true, lastSeenAt: minutesAgo(4), installedAt: '2026-06-20T19:30:00Z', timezone: TZ_MX, tags: ['mall', 'termica'] },
  { id: ID.machine.bogota, code: 'FR-BOG-001', name: 'Documental Andino Norte', organizationId: ID.org.fotorapida, locationId: ID.location.mallBogota, regionId: ID.region.bogota, hardwareProfileId: ID.hardwareProfile.docStation, blueprintId: ID.blueprint.docUniversity, status: 'active', capabilities: caps(CAPS.docStation), printers: [PRINTERS.photo4x6], softwareVersion: SOFTWARE_VERSIONS.v010, online: true, lastSeenAt: minutesAgo(2), installedAt: '2026-05-01T16:00:00Z', timezone: TZ_BOG, tags: ['mall', 'colombia'] },
];
...
export function buildMachines(): Machine[] {
  return seeds.map((seed) =>
    Machine.parse({
      ...
      code: seed.code,
      name: seed.name,
      organizationId: seed.organizationId ?? ID.org.lumina,
