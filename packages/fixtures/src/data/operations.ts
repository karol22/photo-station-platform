...
export function buildIncidents(): Incident[] {
  return [
    Incident.parse({
      id: ID.incident.premiumPaper, code: 'PAP-7Q2', machineId: ID.machine.premium, locationId: ID.location.mallCdmx, organizationId: ID.org.lumina, severity: 'high', category: 'consumables',
    ...
    Incident.parse({
      id: ID.incident.cinemaCamera, code: 'CAM-3KD', machineId: ID.machine.cinema, locationId: ID.location.cinema, organizationId: ID.org.lumina, franchiseId: ID.franchise.bajio, severity: 'critical', category: 'camera',
    ...
    Incident.parse({
      id: ID.incident.terminalThermal, code: 'THR-9XA', machineId: ID.machine.terminal, locationId: ID.location.mallCdmx, organizationId: ID.org.lumina, severity: 'medium', category: 'printer',
...
export function buildInternalDocuments(): InternalDocument[] {
  return [
    InternalDocument.parse({ id: ID.document.changePaper, organizationId: ID.org.lumina, title: L('Cómo cambiar el papel fotográfico', 'How to change photo paper'), body: '1. Abre el gabinete con la llave.\n2. Retira el cartucho vacío.\n3. Inserta el paquete nuevo con la cara brillante hacia arriba.\n4. Ejecuta impresión de prueba desde el panel técnico.', attachedTo: { type: 'hardwareProfile', id: ID.hardwareProfile.docStation }, tags: ['papel', 'mantenimiento'], ...audit(ID.user.adminLumina) }),
    ...
    InternalDocument.parse({ id: ID.document.installChecklist, organizationId: ID.org.lumina, title: L('Checklist de instalación', 'Installation checklist'), body: 'Corriente dedicada, red con salida a internet, nivelación, prueba de cámara, prueba de impresión, enrolamiento con código.', attachedTo: { type: 'installation' }, tags: ['instalacion'], ...audit(ID.user.adminLumina) }),
  ];
}

export function buildAnnouncements(): Announcement[] {
  return [
    Announcement.parse({ id: ID.announcement.christmas2026, organizationId: ID.org.lumina, title: L('Campaña Navidad 2026', 'Christmas 2026 campaign'), body: L('La campaña obligatoria de navidad se activa el 1 de diciembre en Norte y Centro. El pie de pantalla es editable por franquicia.', 'The mandatory Christmas campaign starts December 1 in Norte and Centro. The footer text is editable per franchise.'), publishedAt: daysAgo(2), expiresAt: daysFromNow(90), audienceFranchiseIds: [ID.franchise.norte], pinned: true, ...audit(ID.user.adminLumina) }),
    Announcement.parse({ id: ID.announcement.release020, organizationId: ID.org.lumina, title: L('Release 0.2.0 en piloto', 'Release 0.2.0 in pilot'), body: L('Las máquinas con etiqueta piloto reciben 0.2.0 esta semana en ventana nocturna.', 'Machines tagged pilot receive 0.2.0 this week during the night window.'), publishedAt: daysAgo(5), audienceFranchiseIds: [], pinned: false, ...audit(ID.user.owner) }),
  ];
}
