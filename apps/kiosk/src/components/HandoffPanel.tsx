...
function QrMatrix({ modules, size = 220 }: { modules: boolean[][]; size?: number }): ReactElement | null {
...
export function HandoffPanel({ handoff, onSimulate, canSimulate }: {
  ...
  const modules = handoff.url ? qrMatrix(handoff.url) : undefined;
  ...
  return (
    <div className="kiosk-handoff" data-testid="handoff">
      <p className="kiosk-lead" style={{ margin: 0 }}>
        {t(`kiosk.handoff.instruction.${instruction}`)}
      </p>
      {scanning ? (
