...
| 45 | Referencia investigada de las fotocabinas de Corea, Japón y China | hecho | `ops/campanas/rediseno-visual-kiosco/REFERENCIA-ASIA.md`: cinco lentes con fuentes y cifras (8 tomas para 4 huecos, 8–10 s por toma, sesión de 3–5 min) | 2026-09-09 |
| 46 | Auditoría del kiosco con archivo y línea | hecho | `ops/campanas/rediseno-visual-kiosco/AUDITORIA.md`: diseño, interacción y casos esquina; 36 hallazgos sólo en casos esquina | 2026-09-09 |
| 47 | El recorrido social dispara de más y elige una vez | hecho | `prd_tira_amigos` con `captureCount: 6` y selección de 4; un toque en «Empezar» completa la tanda sin volver a tocar, verificado en el navegador de `/session/capture` a `/session/review` | 2026-09-09 |
| 48 | El tiempo de inactividad no corre mientras la persona posa | hecho | `useIdleHold` retiene el temporizador; `pnpm --filter @psp/kiosk test` cubre el contador de retenciones | 2026-09-09 |
| 49 | Una sesión con dinero o fotografías no se cancela sola | hecho | `idleExpiryAction` en `@psp/domain` con 7 pruebas; cada pantalla de sesión declara su `onAutoAdvance` | 2026-09-09 |
| 50 | La cabina suena | hecho | `packages/ui/src/kiosk/sound.ts` sintetiza cuenta, obturador y celebración sin archivos de audio; 13 pruebas; volumen desde `kiosk.volume`, mudo en cero | 2026-09-09 |
| 51 | Elegir va antes que editar y el estilo es del conjunto | hecho | `stagesForProduct` pone `selecting` antes de `editing`; `bestCaptures` premarca las mejores; la edición social aplica a las cuatro fotos | 2026-09-09 |
| 52 | La sesión no se hereda entre personas | hecho | `releaseSession` cierra en el aparato y reintenta; con cierre pendiente la atracción no ofrece empezar; verificado: cancelar deja `GET /sessions/active` vacío y la siguiente sesión arranca | 2026-09-09 |
| 53 | Dirección visual elegida y aplicada a las ocho pantallas | hecho | `ops/campanas/rediseno-visual-kiosco/PLAN-VISUAL.md` fija la dirección; las cinco bandas caen en 120·1180·1480·1780·1920 en todas, medido en el navegador a 1080×1920 | 2026-09-09 |
| 54 | Tipografía propia empaquetada, sin red | hecho | cuatro papeles en `apps/kiosk/public/fonts/` con su OFL, 180 KB; `scripts/fetch-fonts.sh` los reproduce y verifica la cobertura del español leyendo la tabla `cmap`; espécimen en `ops/campanas/rediseno-visual-kiosco/ESPECIMEN.html` | 2026-09-09 |
| 55 | La marca se dibuja como la lámina: nubes de lóbulos, montón y arco | hecho | `packages/ui/src/kiosk/{BlobFace,BlobPile,BlobFrame}.tsx`; `pnpm --filter @psp/ui test` → 80 pruebas; `ops/campanas/rediseno-visual-kiosco/LAMINA-DE-MARCA.md` | 2026-09-09 |
| 56 | La marquesina es el único reloj del recorrido social | hecho | `packages/ui/src/kiosk/Marquee.tsx` con nueve pruebas; sustituye a `TimeoutBar` con números en atracción, elegir, captura, revisión y edición | 2026-09-09 |
| 57 | Revisar y elegir son una sola pantalla | hecho | `stagesForProduct` deja de emitir `selecting`; la ruta sobrevive para recuperar sesiones guardadas; recorrido verificado: la revisión llega con cuatro de seis ya elegidas | 2026-09-09 |
| 58 | Accesorios que se colocan solos sobre el rostro | hecho | `packages/vision/src/face-anchors.ts` con 17 pruebas; medido contra 34 fotografías reales: lentes 17/17, bigote 16/17; evidencia y límites en `ops/campanas/rediseno-visual-kiosco/evidencia-accesorios/` | 2026-09-09 |
| 59 | Texto sobre la foto | hecho | `captionOp`/`renderTextRaster` en `@psp/imaging`; frases de la marca y fecha en un toque, teclado de nombre detrás de una tecla | 2026-09-09 |
| 60 | Recorrido social completo ejercido de punta a punta | hecho | reposo → elegir → pago → ráfaga de seis sin tocar nada → revisión → estilos → composición → confirmación → impresión → cierre, y `GET /sessions/active` vacío al terminar | 2026-09-09 |
| 61 | Pendiente: verlo en el aparato real | pendiente | nada se ha visto en Android, ni en un panel a 300 cd/m², ni a cuatro metros; la prueba de los cuatro metros del plan sigue sin hacerse | |
| 62 | Pendiente: los aretes cuelgan de la mandíbula, no de la oreja | pendiente | la malla facial se acaba en el contorno del rostro y no tiene oreja; medido y escrito en la evidencia | |
